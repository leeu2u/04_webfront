...
function fnTest3(){
    ...
    //문서내에 존재하는 모든 li 요소를 얻어와 배열 형식으로
    //반환하고 있음
...
function fnTest5() {
    ...
    if(str != "") { // 체크된것이 하나라도 있다면
        result5.innerText = str;

        result5.innerHTML += "<br><br>선택된 개수 : " + count;
        // +=을 쓰는 이유는 위에 거에서 더해준다는 개념, +가 없다면 위에꺼가 무시당함
    }
    else {
...
function readValue(){// 입력된 값을 읽어들여 채팅창에 출력
    ...
    if(input.value.trim().length > 0) {
        ...
        // 요소.scrollTop = 위치; : 스크롤을 특정 위치로 이동한다.
...
function inputEnter(){
    ...
    //keydown : 모든키가 눌렸을 때
    //keypress :  영문/숫자/특수문자/띄어쓰기 눌렸을 때
